...
App::import('Vendor', array('Payment.Bitcoin'));
if (!defined('BITCOIN_CERTIFICATE')) {
	define('BITCOIN_CERTIFICATE', CONFIGS.'server.cert');
}
...
class BitcoinLib extends Bitcoin {
	...
	function __construct($settings = array()) {
		$this->defaults = am($this->defaults, (array)Configure::read('Bitcoin'));
		$this->settings = am($this->defaults, $settings);
	...
	function getAddressesByAccount($account = null) {
		if (!$this->settings['daemon']) {
			return array();
		}
		if ($account === null) {
	...
	function getAccountAddress($account = null) {
		if (!$this->settings['daemon']) {
			return false;
		}
		if ($account === null) {
	 ...
	 * 2011-07-20 ms
	 */
	function setAccount($address, $account = null) {
		return $this->C->setaccount($address, $account = null);
	}

	function backupWallet($dest) {
	 ...
	 * 2011-07-16 ms
	 */
	function getReceivedByAddress($address, $minconf = 1) {
		if ($minconf === null) {
	...
	function listReceivedByAddress($minconf = 1, $includeempty = FALSE) {
		if ($minconf === null) {
			$minconf = $this->settings['minconf'];
		}
	...
	function listReceivedByAccount($minconf = 1, $includeempty = FALSE) {
		if ($minconf === null) {
			$minconf = $this->settings['minconf'];
		}
	...
	function listReceivedByLabel($minconf = 1, $includeempty = FALSE) {
		if ($minconf === null) {
			$minconf = $this->settings['minconf'];
		}
	 ...
	 * 2011-07-21 ms
	 */
	function listTransactions($account = null, $type = null) {
		if (!$this->settings['daemon']) {
			return array();
		}
		if ($account === null) {
	 ...
	 * 2011-07-19 ms
	 */
	function validateTransaction($txid, $isMine = null) {
	 ...
	 * 2011-07-18 ms
	 */
	function validateAddress($address, $isMine = null) {
  	/*
	...
	function getNewAddress($account = null) {
		if (!$this->settings['daemon']) {
			return false;
		}
		if ($account === null) {
	...
	function sendFrom($fromAccount = null, $toAddress, $amount, $minconf = 1, $comment = NULL, $comment_to = NULL) {
		if ($fromAccount === null) {
	...
	function sendToAddress($address, $amount, $comment = NULL, $comment_to = NULL) {
		if (!$this->settings['daemon']) {
			return false;
		}
	 ...
	 * 2011-07-20 ms
	 */
	function move($fromAccount = null, $toAccount, $amount, $minconf = 1, $comment = NULL) {
		if (!$this->settings['daemon']) {
			return false;
		}
		if ($fromAccount === null) {
...
from the amount of BTC sent by the person owning the address
	 * 2011-07-16 ms
	 */
	function getTotalSentByAddress($address) {
...
hash: transactions in blocks up to and including this block will not be returned.
	 * 2011-07-16 ms
	 */
	function myTransactions($address, $block = null) {
		if ($block) {
			$address .= '/'.$block;
		}
...
transactions comprising the balance
	 ...
	 * 2011-07-16 ms
	 */
	function _getReceivedByAddress($address, $minconf = null) {
		if ($minconf) {
			$address .= '/'.$minconf;
		}
